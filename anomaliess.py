...
columns_for_cluster = ['Number_orders', 'Number_doctors', 'Artinibsa', 'ALEXADRICAINE',
                       'ART_PHARMA', 'MEPECAIN', 'Shipping', 'populationsize', 
                       'Number_Clinics', 'Number_Centres', 'seats']

# Perform clustering using k-means
kmeans = KMeans(n_clusters=4, random_state=42)
kmeans.fit(merged_df[columns_for_cluster])

# Predict the cluster labels for each data point
merged_df['Cluster'] = kmeans.predict(merged_df[columns_for_cluster])

# Calculate the distance of each point to its nearest cluster center
merged_df['Distance'] = kmeans.transform(merged_df[columns_for_cluster]).min(axis=1)

# Define a threshold to classify anomalies
threshold = np.percentile(merged_df['Distance'], 95)  # For example, consider the top 5% as anomalies

# Identify anomalies
anomalyes = merged_df[merged_df['Distance'] > threshold]
...
for label in range(kmeans.n_clusters):
    plt.scatter(merged_df.loc[merged_df['Cluster'] == label, 'Weeks'], merged_df.loc[merged_df['Cluster'] == label, 'Number_orders'],
                label=f'Cluster {label}')

# Plot the anomalies as red points
plt.scatter(anomalyes['Weeks'], anomalyes['Number_orders'], color='blue', label='Anomalies')
...
plt.legend()
plt.show()
numeric_features = merged_df.select_dtypes(include=['int', 'float']).columns.tolist()

# Specify the features you want to include in the analysis
numeric_features = merged_df.select_dtypes(include=['int', 'float']).columns.tolist()

# Specify the features you want to include in the analysis
selected_features = numeric_features  # You can customize this based on your needs

# Instantiate the Isolation Forest model
model = IsolationForest()

# Fit the Isolation Forest model to the selected features
model.fit(merged_df[selected_features])

# Train a Random Forest classifier as a surrogate model to estimate feature importance
rf_model = RandomForestClassifier()
rf_model.fit(merged_df[selected_features], model.predict(merged_df[selected_features]))
...
print("Feature Importance Scores (Estimated by Random Forest):")
for feature, importance_score in zip(selected_features, feature_importance_scores):
