# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 14:37:29 2024
...
le = LabelEncoder()
merged_df["governorateName_encoded"] = le.fit_transform(merged_df["governorateName"])
merged_df.drop(columns='governorateName',inplace=True)
#merged_df.drop(columns='Weeks',inplace=True)
# Separate features (X) and target variable (y)
X = merged_df.drop("MEPECAIN", axis=1)
y =merged_df["MEPECAIN"]
...
plt.title('Correlation between Predicted y and Exact y')

# Add a diagonal line for reference
plt.plot([min(y_pred), max(y_pred)], [min(y_pred), max(y_pred)], color='green')
