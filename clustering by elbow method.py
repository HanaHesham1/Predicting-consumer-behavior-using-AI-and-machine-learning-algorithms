...
#Articaine=sum_values_by_unique_column(df, 'governorateName', 'quantity','Articaine Hydrochloride 1:100,000, 50 Cartridges')
Lidocaine=sum_values_by_unique_column(df, 'governorateName', 'quantity','Lidocaine Hydrochloride 1:80,000, 50 Cartridges')
...
#merged_df = pd.merge(merged_df, Articaine,on=['governorateName', 'Weeks'], how='left')
merged_df = pd.merge(merged_df, Lidocaine,on=['governorateName', 'Weeks'], how='left')
merged_df.rename(columns={'quantity': 'Lidocaine'}, inplace=True)
#Septanest=sum_values_by_unique_column(df, 'governorateName', 'quantity','Septanest N Articaine hydrochloride 4% with 1:200 epinephrine')
#merged_df = pd.merge(merged_df, Septanest,on=['governorateName', 'Weeks'], how='left')
#merged_df.rename(columns={'quantity': 'Septanest'}, inplace=True)
#print(merged_df)
...
plt.plot(weeks, alexandican, marker='o', label='Alexadricaine')
plt.plot(weeks, ART_pharma, marker='o', label='ART_pharma')
plt.plot(weeks, MEPECAIN, marker='o', label='MEPECAIN')
plt.xlabel('Weeks')
plt.ylabel('Quantity')
plt.title('Line Graphs: Artinibsa, Alexridicaine, art pharma, mepecain')
plt.legend()
plt.grid(True)
plt.show()
reshaped_words = [get_display(arabic_reshaper.reshape(word)) for word in merged_df['governorateName']]
...
plt.title('Number of Doctors made orders ')
...
plt.title(f'Correlation: {correlation:.2f}')
plt.show()
def add_study_column(df):
    # Create a boolean mask based on the conditions provided
    mask = ((df['Weeks'].between(1, 20)) | (df['Weeks'].between(40, 66)))
    
    # Create a new column 'study' with boolean values based on the mask
    df['study'] = mask
    
    return df

merged_df=add_study_column(merged_df)
merged_df.drop(columns='governorateName',inplace=True)
def elbow_method_dbscan(df, min_samples_range=(2, 10), eps_range=(0.1, 1.0)):
    silhouette_scores = []
    ...
    for min_samples in range(*min_samples_range):
        for eps in np.arange(*eps_range, 0.1):
            ...
            if len(set(labels)) > 1:  # Ensure more than one cluster is formed
                silhouette_scores.append((min_samples, eps, silhouette_score(df, labels)))

    silhouette_scores = np.array(silhouette_scores)
    if silhouette_scores:
      best_parameters = silhouette_scores[np.argmax([score[2] for score in silhouette_scores])]
      print("Best Parameters:", best_parameters)
    else:
      print("No valid clusters found. Unable to compute silhouette scores.")
    
    plt.plot(silhouette_scores[:, 0], silhouette_scores[:, 2])
    plt.xlabel('Min Samples')
    plt.ylabel('Silhouette Score')
    plt.title('Elbow Curve for DBSCAN (Best Parameters: Min Samples={}, Eps={})'.format(int(best_parameters[0]), best_parameters[1]))
    plt.show()

# Example usage:
# Assuming merged_df is defined
elbow_method_dbscan(merged_df)